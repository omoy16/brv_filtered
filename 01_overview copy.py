...
# ランダムフォレストモデル（n_estimators=300, max_depth=5）
print("\n=== ランダムフォレストモデルの学習 ===")
...
# ランダムフォレストでは複数の木の集合体となるため、単一のルール表示は省略します。
...
# 確率を0.05刻みなどで丸めて集計すると、傾向が見やすくなります
prob_summary = pd.DataFrame({
    "Predicted_Prob": y_proba.round(2),
...
top20_count = int(len(result) * 0.2)

top20 = result.head(top20_count)

print("件数:", len(top20))
print("勝率:", top20["Actual"].mean())
...
for col in target_cols:
    ...
    print(f"{col:<18} | 上位10%: {top_mean:>8.4f} | 全体: {all_mean:>8.4f} | 差: {top_mean - all_mean:>+8.4f}")
